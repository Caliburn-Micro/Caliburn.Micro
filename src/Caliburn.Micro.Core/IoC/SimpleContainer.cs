...
public class SimpleContainer {
    ...
    /// <summary>
    /// Gets or sets a value indicating whether to enable recursive property injection for all resolutions.
    /// </summary>
    ...
    public object GetInstance(Type service, string key) {
        ContainerEntry entry = GetEntry(service, key);
        if (entry != null) {
            object instance = entry.Single()(this);
    ...
    public void BuildUp(object instance) {
        ...
        foreach (PropertyInfo property in properties) {
            object value = GetInstance(property.PropertyType, null);
            if (value == null) {
                continue;
            }
    ...
    protected virtual object ActivateInstance(Type type, object[] args) {
        object instance = args.Length > 0 ? System.Activator.CreateInstance(type, args) : System.Activator.CreateInstance(type);
        Activated(instance);
    ...
    private ContainerEntry GetEntry(Type service, string key)
        => service == null
            ? _entries.FirstOrDefault(x => x.Key == key)
            : key == null
