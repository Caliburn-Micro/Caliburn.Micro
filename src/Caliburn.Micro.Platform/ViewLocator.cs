...
using TextBlock = Xamarin.Forms.Label;
...
#endif
{
    ...
    public static class ViewLocator {
        ...
        /// <summary>
        ///   Gets or sets separator used when resolving View names for context instances.
        /// </summary>
        public static string ContextSeparator { get; set; }
            = ".";

        /// <summary>
        /// Gets or sets Name Transformer.
        /// </summary>
        public static NameTransformer NameTransformer { get; set; }
            = new NameTransformer();

        /// <summary>
        ///   Gets or sets func to retrieves the view from the IoC container or tries to create it if not found.
        /// </summary>
        /// <remarks>
        ///   Pass the type of view as a parameter and recieve an instance of the view.
        /// </remarks>
        public static Func<Type, UIElement> GetOrCreateViewType { get; set; }
            = viewType => {
                if (IoC.GetAllInstances(viewType).FirstOrDefault() is UIElement view) {
                ...
                if (viewType.IsInterface || viewType.IsAbstract || !typeof(UIElement).IsAssignableFrom(viewType)) {
                    return new TextBlock { Text = string.Format(CultureInfo.InvariantCulture, "Cannot create {0}.", viewType.FullName) };
                }
#else
                TypeInfo viewTypeInfo = viewType.GetTypeInfo();
                TypeInfo uiElementInfo = typeof(UIElement).GetTypeInfo();
        ...
        /// <summary>
        /// Gets or sets func to modifies the name of the type to be used at design time.
        /// </summary>
        public static Func<string, string> ModifyModelTypeAtDesignTime { get; set; }
            = modelTypeName => {
                if (!modelTypeName.StartsWith("_", StringComparison.OrdinalIgnoreCase)) {
        ...
        /// <summary>
        /// Gets or sets func to transforms a ViewModel type name into all of its possible View type names. Optionally accepts an instance
        /// of context object.
        /// </summary>
        ...
        /// </remarks>
        public static Func<string, object, IEnumerable<string>> TransformName { get; set; }
            = (typeName, context) => {
                ...
                if (useNameSuffixesInMappings) {
                    ...
                    string synonymregex = "(" + string.Join("|", ViewSuffixList.ToArray()) + ")";
                    grpsuffix = RegExHelper.GetCaptureGroup("suffix", synonymregex);
                }

                const string grpbase = @"\${basename}";
                string patternregex = string.Format(CultureInfo.InvariantCulture, nameFormat, grpbase, grpsuffix) + "$";
                ...
                return NameTransformer.Transform(typeName, getReplaceString).Where(n => n.EndsWith(contextstr, StringComparison.OrdinalIgnoreCase));
            };

        /// <summary>
        ///   Gets or sets func to locates the view type based on the specified model type.
        /// </summary>
        /// <returns>The view.</returns>
        /// <remarks>
        ///   Pass the model type, display location (or null) and the context instance (or null) as parameters and receive a view type.
        /// </remarks>
        public static Func<Type, DependencyObject, object, Type> LocateTypeForModelType { get; set; }
            = (modelType, displayLocation, context) => {
                ...
                IEnumerable<string> viewTypeList = TransformName(viewTypeName, context);
                Type viewType = AssemblySource.FindTypeByNames(viewTypeList);
        ...
        /// <summary>
        ///   Gets or sets func to locates the view for the specified model type.
        /// </summary>
        /// <returns>The view.</returns>
        /// <remarks>
        ///   Pass the model type, display location (or null) and the context instance (or null) as parameters and receive a view instance.
        /// </remarks>
        public static Func<Type, DependencyObject, object, UIElement> LocateForModelType { get; set; }
            = (modelType, displayLocation, context) => {
                Type viewType = LocateTypeForModelType(modelType, displayLocation, context);

                return viewType == null
                           ? new TextBlock { Text = string.Format(CultureInfo.InvariantCulture, "Cannot find view for {0}.", modelType) }
                           : GetOrCreateViewType(viewType);
            };

        /// <summary>
        ///   Gets or sets func to locates the view for the specified model instance.
        /// </summary>
        /// <returns>The view.</returns>
        /// <remarks>
        ///   Pass the model instance, display location (or null) and the context (or null) as parameters and receive a view instance.
        /// </remarks>
        public static Func<object, DependencyObject, object, UIElement> LocateForModel { get; set; }
            = (model, displayLocation, context) => {
                if (model is IViewAware viewAware &&
                    viewAware.GetView(context) is UIElement view) {
#if !WINDOWS_UWP && !XFORMS && !MAUI
                    if (view is not Window windowCheck || (!windowCheck.IsLoaded && !(new WindowInteropHelper(windowCheck).Handle == IntPtr.Zero))) {
        ...
        /// <summary>
        /// Gets or sets functo transforms a view type into a pack uri.
        /// </summary>
        public static Func<Type, Type, string> DeterminePackUriFromType { get; set; }
            = (viewModelType, viewType) => {
#if !WINDOWS_UWP && !XFORMS && !MAUI
                string assemblyName = viewType.Assembly.GetAssemblyName();
                string applicationAssemblyName = Application.Current.GetType().Assembly.GetAssemblyName();
#else
                string assemblyName = viewType.GetTypeInfo().Assembly.GetAssemblyName();
                string applicationAssemblyName = Application.Current.GetType().GetTypeInfo().Assembly.GetAssemblyName();
#endif
                string viewTypeName = viewType.FullName;

                if (viewTypeName.StartsWith(assemblyName, StringComparison.OrdinalIgnoreCase)) {
                    viewTypeName = viewTypeName.Substring(assemblyName.Length);
                }

                string uri = viewTypeName.Replace(".", "/") + ".xaml";
                return !applicationAssemblyName.Equals(assemblyName, StringComparison.OrdinalIgnoreCase)
                    ? "/" + assemblyName + ";component" + uri
                    : uri;
            };
        ...
        public static void RegisterViewSuffix(string viewSuffix) {
            if (ViewSuffixList.Any(s => s == viewSuffix)) {
                return;
            }
        ...
        public static void AddTypeMapping(string nsSourceReplaceRegEx, string nsSourceFilterRegEx, string[] nsTargetsRegEx, string viewSuffix = "View") {
            ...
            string repsuffix = useNameSuffixesInMappings ? viewSuffix : string.Empty;
            const string basegrp = "${basename}";

            foreach (string t in nsTargetsRegEx) {
                replist.Add(t + string.Format(CultureInfo.InvariantCulture, nameFormat, basegrp, repsuffix));
            }
            ...
            NameTransformer.AddRule(
                string.Concat(nsSourceReplaceRegEx, string.Format(CultureInfo.InvariantCulture, nameFormat, rxbase, rxsuffix), "$"),
                replist.ToArray(),
                rxsrcfilter);
        }
...
#else
            MethodInfo method = element.GetType().GetTypeInfo()
