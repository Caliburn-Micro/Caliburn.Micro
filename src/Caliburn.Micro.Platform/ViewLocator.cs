...
#endif
{
    ...
    public static class ViewLocator
    {
        ...
        // This is the configuration of how viewlocation behaves
        private static ViewCreationConfiguration viewCreationConfiguration;
        ...
        static ViewLocator()
        {
            ...
            ViewLocator.viewCreationConfiguration = new ViewCreationConfiguration();
        }
        ...
        /// <summary>
        /// Specifies how views will be created.
        /// By default viewlocation is done by calling IoC.GetInstance which you can use to supply the view
        /// from a DI container of your choice. If you return <see langword="null"/> Caliburn will try to create
        /// the view for you. Exception handling however should be done by yourself! If Caliburn is also unable to
        /// create the view, an exception is raised 
        ...
        public static void ConfigureViewCreation(ViewCreationConfiguration config)
        {
            ...
            ViewLocator.viewCreationConfiguration = config;
        }
        ...
        {
            switch (ViewLocator.viewCreationConfiguration.ViewCreationBehavior)
            {
                case ViewCreationBehavior.UseDIContainer:
                    return ViewLocator.GetOrCreateViewFromDIContainer(viewType);
                case ViewCreationBehavior.UseV3StyleLocation:
                    return ViewLocator.GetOrCreateViewTypeV3Style(viewType);
                case ViewCreationBehavior.DontUseDi:
                    return ViewLocator.CreateView(viewType);
                default:
                    throw new ArgumentOutOfRangeException(nameof(ViewLocator.viewCreationConfiguration.ViewCreationBehavior));
            }
        };

        private static UIElement GetOrCreateViewFromDIContainer(Type viewType)
        {
            var view = IoC.GetInstance(viewType, null) as UIElement;
        ...
        private static UIElement CreateView(Type viewType) {
            var result = ViewLocator.CanWeCreateTheView(viewType);

            if (result.ViewIsCreatable) {
                var view = (UIElement) System.Activator.CreateInstance(viewType);

                return InitializeAndReturnView(view);
            }

            if (ViewLocator.viewCreationConfiguration.ShowEmptyViewWhenCreationFailed) {
#if !WINDOWS_UWP && !XFORMS
                return new TextBlock { Text = result.Message };
#else
                return new TextBlock { Text = result.Message };
#endif                
            }

            throw new ViewCantBeCreatedException(result.Message);
        }

        private static ViewCreatableResult CanWeCreateTheView(Type viewType) {
            ...
            if (viewTypeInfo.IsInterface || viewTypeInfo.IsAbstract || !uiElementInfo.IsAssignableFrom(viewTypeInfo)) {
                return new ViewCreatableResult(viewIsCreatable: false,
                    message: $"Cannot create {viewType.FullName} because the type is abstract or not of type `UIElement`.");
            }

            var constructors = (
                from ctor in viewTypeInfo.DeclaredConstructors
                where ctor.IsPublic
                select new
                {
                    NumberOfParameters = ctor.GetParameters().Length,
                }).ToArray();

            if (constructors.Length != 1 || constructors.Single().NumberOfParameters != 0) {
                return new ViewCreatableResult(viewIsCreatable: false,
                    message: $"Cannot create {viewType.FullName} because the type has no default public constructor. Multiple constructors are not supported!");
            }

            return new ViewCreatableResult(true);
        }

        private static UIElement InitializeAndReturnView(UIElement view)
        {
        ...
        private class ViewCreatableResult {
            public ViewCreatableResult(bool viewIsCreatable, string message = null) {
                if (!viewIsCreatable && message == null) {
                    throw new InvalidOperationException("If we can't create the view, please supply a valid " +
                                                        "message for the user!");
                }

                this.ViewIsCreatable = viewIsCreatable;
                this.Message = message;
            }
        
            public bool ViewIsCreatable { get; }
